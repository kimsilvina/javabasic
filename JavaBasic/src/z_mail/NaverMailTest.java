	...
	public static void main(String[] args) {
		...
		Session session = Session.getDefaultInstance(p,  new Authenticator() {
			...
			protected PasswordAuthentication getPasswordAuthentication() {				
				return new PasswordAuthentication("xxxxx@naver.com", "네이버비밀번호");
				// 네이버 보내는 아이디와 비밀번호
			}
		});
		...
		try{
			...
			InternetAddress from = new InternetAddress("xxxxx@naver.com"); //발신자 아이디
			// 이메일 발신자
			msg.setFrom(from);
			// 이메일 수신자
			InternetAddress to = new InternetAddress("yyyyyy@naver.com");
